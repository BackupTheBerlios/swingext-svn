...
/**
 * The SimpleAccessoryManager allows you to install custom accessories into a JFileChooser at different locations.
 * This implementation reserves four fields for your extensions, north, south, east and west. Only one accessory
 * can be installed into one of those fields at a time, however, the components you install may be arbitrarily complex.
 * <p/>
 * <hr/>
...
public class SimpleAccessoryManager extends AccessoryManager {
     ...
     * @param position  desired location of the component. Has to be one of
    ...
    public JComponent setAccessory(final JComponent accessory, final int position) {
        ...
        if (_accessories.get(position) != null) {
            previousAccessory = _accessories.get(position);
            _accessoryPane.remove(previousAccessory);
            _accessories.remove(position);
        }
        ...
        _accessories.put(position, accessory);
